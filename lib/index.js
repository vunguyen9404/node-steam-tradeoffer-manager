...
TradeOfferManager.prototype.setCookies = function(cookies, familyViewPin, callback) {
    ...
    try {
        var loginSecureCookie = cookies.filter(function (cookie) { return cookie.indexOf('steamLoginSecure=') === 0 })[0];
        if (!loginSecureCookie) throw new Error('steamLoginSecure cookie not found');

        var accessToken = decodeURIComponent(loginSecureCookie.split(';')[0].split('=')[1]).split('||')[1];
        if (!accessToken) throw new Error('Access token not found');

        this.accessToken = accessToken;
    } catch (error) {
        if (this.useAccessToken) {
            if (callback) callback(new Error('Access token not found'));
            return;
        }
    }
    ...
    var finish = (error) => {
        if (!error || (error.message === 'No API key created for this account' && this.accessToken)) {
            if (!this.useAccessToken) {
                this._warn(
                    'Api key not found, access token will be used instead for API requests.' +
                    '\nTo disable this warning, create api key for account or set useAccessToken to true in manager options.'
                );
            }
