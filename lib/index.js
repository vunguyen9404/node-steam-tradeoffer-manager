...
const SteamCommunity = require('@vgm-com/steamcommunity');
...
const CEconItem = require('@vgm-com/steamcommunity/classes/CEconItem.js');
 ...
 * @param {boolean} tradableOnly - The Steam application ID of the game for which you want an inventory
 * @param {function} callback
 */
TradeOfferManager.prototype.getMyInventoryContent = function(partnerID, appID, contextID, tradableOnly, callback) {
	var self = this;
	...
	function get(inventory, currency, start) {
		self._community.httpRequestGet({
			"uri": `https://steamcommunity.com/profiles/${partnerID.getSteamID64()}/inventory/json/${appID}/${contextID}/`,
			"headers": {
			...
			if (err.message.includes("403") && !isJsonString(body.toString())) {
				if (body.toString().includes("{\"success\":false,\"Error\":\"Unsupported request\"}")) {
					let bodyFix = body.toString().replace("{\"success\":false,\"Error\":\"Unsupported request\"}", "");
					err = null;
					body = JSON.parse(bodyFix);
				}
			}
			if (err) {
				callback(err);
 ...
 * @param {number} startAsset
 * @param {function} callback
 */
TradeOfferManager.prototype.getPartnerInventoryWithPaginationAndProxy = function(partnerID, appID, contextID, proxyList, startAsset, callback) {
	var self = this;
 ...
 * @param callback
 */
TradeOfferManager.prototype.getUserInventoryContentsWithProxy = function(sid, appid, contextid, tradableOnly, proxyList, count, callback) {
	this._community.getUserInventoryContentsWithProxy(sid, appid, contextid, tradableOnly, this._languageName || "english", proxyList, count, callback);
};
 ...
 * @param callback
 */
TradeOfferManager.prototype.getUserInventoryContentsPaginationWithProxy = function(sid, appid, contextid, tradableOnly, proxyList, count, startAsset, callback) {
	this._community.getUserInventoryContentsPaginationWithProxy(sid, appid, contextid, tradableOnly, this._languageName || "english", proxyList, count, startAsset, callback);
};
